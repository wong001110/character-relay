...
| Knowledge Fabric scope/content/interpretation/query/context/projection/adapters | `persistence/knowledge_fabric_*`, `knowledge_fabric_*adapter.py`, `knowledge_fabric_*sync.py`, `knowledge_fabric_pinned_fetcher.py`, `knowledge_fabric_external_sync_scheduler.py`, `knowledge_fabric_invalidation_worker.py`, ingestion/query/context/epistemic/invalidation policy modules, `knowledge_object_storage.py`, `api/routes/knowledge_fabric.py` | `tests/test_knowledge_fabric_phase2.py` through `test_knowledge_fabric_phase5.py`, query/invalidation policy, worker, projection/document/git/website/Atom/schedule/pinned-fetcher/epistemic tests, PostgreSQL foundation tests | `docs/knowledge-fabric-architecture.md`, active Phase 12 plan |
