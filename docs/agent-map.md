...
| Knowledge Fabric scope/content/interpretation/query | `persistence/knowledge_fabric_*`, `knowledge_fabric_policy.py`, `knowledge_fabric_ingestion*.py`, `knowledge_fabric_interpretation_policy.py`, `knowledge_fabric_query*.py`, `knowledge_object_storage.py`, `api/routes/knowledge_fabric.py` | `tests/test_knowledge_fabric_phase2.py` through `test_knowledge_fabric_phase5.py`, focused Knowledge Fabric policy tests, PostgreSQL foundation tests | `docs/knowledge-fabric-architecture.md`, active Phase 5 plan |
