...
class KnowledgeFabricExternalScheduleMigration:
    """Record the additive opt-in external-schedule schema after ORM bootstrap."""
    ...
    def run(self) -> None:
        ...
        if dialect == "postgresql":
            with self.database.engine.begin() as connection:
                connection.execute(
                    text("SELECT pg_advisory_xact_lock(hashtext(:key))"),
                    {"key": KNOWLEDGE_FABRIC_EXTERNAL_SCHEDULE_REVISION},
                )
                self._record(connection, database_kind="postgresql")
            return
        with self.database.session() as session:
            if (
                ...
                session.add(
                    DatabaseSchemaMigrationRecord(
                        revision=KNOWLEDGE_FABRIC_EXTERNAL_SCHEDULE_REVISION,
                        database_kind=dialect,
                    )
                )
                session.commit()
...
class KnowledgeFabricCurrentEntryMigration:
    """Record additive current-entry mapping schema after ORM bootstrap."""
    ...
    def run(self) -> None:
        ...
        if dialect == "postgresql":
            with self.database.engine.begin() as connection:
                connection.execute(
                    text("SELECT pg_advisory_xact_lock(hashtext(:key))"),
                    {"key": KNOWLEDGE_FABRIC_CURRENT_ENTRY_REVISION},
                )
                self._record(connection, database_kind="postgresql")
            return
        with self.database.session() as session:
            if (
                session.get(DatabaseSchemaMigrationRecord, KNOWLEDGE_FABRIC_CURRENT_ENTRY_REVISION)
                is None
            ):
                session.add(
                    DatabaseSchemaMigrationRecord(
                        revision=KNOWLEDGE_FABRIC_CURRENT_ENTRY_REVISION,
                        database_kind=dialect,
                    )
                )
                session.commit()
...
class KnowledgeFabricSiteCollectionStateMigration:
    """Record additive Site Collection page-state schema after ORM bootstrap."""
    ...
    def run(self) -> None:
        ...
        if dialect == "postgresql":
            with self.database.engine.begin() as connection:
                connection.execute(
                    text("SELECT pg_advisory_xact_lock(hashtext(:key))"),
                    {"key": KNOWLEDGE_FABRIC_SITE_COLLECTION_STATE_REVISION},
                )
                self._record(connection, database_kind="postgresql")
            return
        with self.database.session() as session:
            if (
                session.get(
                    DatabaseSchemaMigrationRecord,
                    KNOWLEDGE_FABRIC_SITE_COLLECTION_STATE_REVISION,
                )
                is None
            ):
                session.add(
                    DatabaseSchemaMigrationRecord(
                        revision=KNOWLEDGE_FABRIC_SITE_COLLECTION_STATE_REVISION,
                        database_kind=dialect,
                    )
                )
                session.commit()
    ...
    def run(self) -> None:
        ...
        if dialect == "postgresql":
            with self.database.engine.begin() as connection:
                connection.execute(
                    text("SELECT pg_advisory_xact_lock(hashtext(:key))"),
                    {"key": KNOWLEDGE_FABRIC_VISUAL_REFERENCE_REVISION},
                )
                self._record(connection, database_kind="postgresql")
            return
        with self.database.session() as session:
            record = session.get(
                DatabaseSchemaMigrationRecord,
                KNOWLEDGE_FABRIC_VISUAL_REFERENCE_REVISION,
            )
            if record is None:
                session.add(
                    DatabaseSchemaMigrationRecord(
                        revision=KNOWLEDGE_FABRIC_VISUAL_REFERENCE_REVISION,
                        database_kind=dialect,
                    )
                )
                session.commit()
...
class KnowledgeFabricCharacterPolicyMigration:
    """Record additive Phase 10 Character corpus policy schema after ORM bootstrap."""
    ...
    def run(self) -> None:
        ...
        if dialect == "postgresql":
            with self.database.engine.begin() as connection:
                connection.execute(
                    text("SELECT pg_advisory_xact_lock(hashtext(:key))"),
                    {"key": KNOWLEDGE_FABRIC_CHARACTER_POLICY_REVISION},
                )
                self._record(connection, database_kind="postgresql")
            return
        with self.database.session() as session:
            if (
                session.get(
                    DatabaseSchemaMigrationRecord,
                    KNOWLEDGE_FABRIC_CHARACTER_POLICY_REVISION,
                )
                is None
            ):
                session.add(
                    DatabaseSchemaMigrationRecord(
                        revision=KNOWLEDGE_FABRIC_CHARACTER_POLICY_REVISION,
                        database_kind=dialect,
                    )
                )
                session.commit()
...
class KnowledgeFabricObjectLifecycleMigration:
    """Record the durable private-object deletion outbox after ORM bootstrap."""
    ...
    def run(self) -> None:
        ...
        if dialect == "postgresql":
            with self.database.engine.begin() as connection:
                connection.execute(
                    text("SELECT pg_advisory_xact_lock(hashtext(:key))"),
                    {"key": KNOWLEDGE_FABRIC_OBJECT_LIFECYCLE_REVISION},
                )
                self._record(connection, database_kind="postgresql")
            return
        with self.database.session() as session:
            if (
                ...
                session.add(
                    DatabaseSchemaMigrationRecord(
                        revision=KNOWLEDGE_FABRIC_OBJECT_LIFECYCLE_REVISION,
                        database_kind=dialect,
                    )
                )
                session.commit()
