...
LEGACY_RUNTIME_DOC_ENV_VAR = re.compile(
    r"\bECHO_MASQUE_(?!LIVE_)[A-Z][A-Z0-9_]*\b"
)


def _legacy_prefix_hits(
    paths: list[Path],
    *,
    pattern: re.Pattern[str] = LEGACY_ENV_VAR,
) -> list[str]:
    hits: list[str] = []
    for path in paths:
...
def test_active_configuration_docs_use_character_relay_environment_namespace() -> None:
    hits = _legacy_prefix_hits(
        list(ACTIVE_CONFIG_DOCS),
        pattern=LEGACY_RUNTIME_DOC_ENV_VAR,
    )
    assert hits == [], (
