...
def runtime_context() -> ToolExecutionContext:
...
def target_for(provider: object) -> PromptModelTarget:
...
def test_prompt_model_explicit_steps_match_bounded_tool_loop() -> None:
    provider = FakeToolProvider()
    target = target_for(provider)
    registry = default_tool_registry()

    turn = asyncio.run(
        target.start_tool_turn(
            "What is 8 * 0.27?",
    ...
    first = asyncio.run(target.advance_tool_model(turn))
    assert first is None
    assert turn.tool_rounds == 1
    assert len(turn.pending_tool_calls) == 1

    assert asyncio.run(target.execute_pending_tools(turn)) == 1
    assert not turn.pending_tool_calls
    assert len(turn.traces) == 1

    second = asyncio.run(target.advance_tool_model(turn))
    assert second is not None
    assert second.text == "The exact result is 2.16."
    assert second.latency_ms == 7
