...
interface Props {
  ...
  onClearCredential: (memberId: string) => Promise<void>;
}
...
export function UtilityGatewayPanel({
  ...
  function patchMember(index: number, values: Partial<UtilityProviderMember>) {
    patch({ members: config.members.map((member, itemIndex) => itemIndex === index ? { ...member, ...values } : member) });
  }
  ...
  function toggleCapability(index: number, capability: UtilityCapability) {
    ...
    const capabilities = active ? member.capabilities.filter((item) => item !== capability) : [...member.capabilities, capability];
    if (capabilities.length) patchMember(index, { capabilities });
  }
  ...
  return (
    <section className="runtime-panel utility-gateway-panel">
      <div className="runtime-heading">
        ...
        <div className={config.enabled ? "runtime-badge ready" : "runtime-badge missing"}>{config.enabled ? "ENABLED" : "OFF"}<small>{config.members.length} providers</small></div>
      </div>
      <p className="section-help">{zh ? "系统级 AI 统一从这里取用。Free Pool member 永远是 FREE ONLY；付费兜底只能走 OpenRouter。Phase 1 只建立配置与 Vault，尚未迁移 Topic / Memory / Media 调用。" : "System AI is managed here. Free Pool members stay FREE ONLY and paid fallback is OpenRouter-only. Phase 1 establishes configuration and Vault storage only; Topic, Memory, and Media are not migrated yet."}</p>

      <div className="utility-gateway-controls">
        <label className="runtime-toggle"><input type="checkbox" checked={config.enabled} onChange={(event) => patch({ enabled: event.currentTarget.checked })} />{zh ? "启用 Utility Gateway" : "Enable Utility Gateway"}</label>
        <label>{zh ? "路由策略" : "Routing strategy"}<select value={config.routing_strategy} onChange={(event) => patch({ routing_strategy: event.currentTarget.value as UtilityGatewayConfig["routing_strategy"] })}><option value="best_available">Best available</option><option value="fixed_priority">Fixed priority</option></select></label>
      </div>

      <div className="utility-provider-list">
        {config.members.map((member, index) => {
          const credential = statusById.get(member.id);
          const persisted = Boolean(credential);
          return (
            <article className="utility-provider-card" key={`${member.id}-${index}`}>
              <header><div><span>FREE MEMBER {String(index + 1).padStart(2, "0")}</span><strong>{member.name || member.id}</strong></div><div><b>FREE ONLY</b><span className={credential?.configured ? "status-chip is-ready" : "status-chip"}>{credential?.configured ? "KEY ✓" : persisted ? "NO KEY" : "UNSAVED"}</span></div></header>
              <div className="runtime-provider-grid">
                ...
                <label>ID<input disabled={persisted} value={member.id} onChange={(event) => patchMember(index, { id: event.currentTarget.value })} /><small>{persisted ? (zh ? "保存后 ID 固定，避免 Vault scope 失联。" : "ID locks after save to keep the Vault scope stable.") : "a-z / 0-9 / _ / -"}</small></label>
              ...
              <div className="utility-capability-grid">{(Object.keys(capabilityLabels) as UtilityCapability[]).map((capability) => <label key={capability}><input type="checkbox" checked={member.capabilities.includes(capability)} onChange={() => toggleCapability(index, capability)} />{capabilityLabels[capability]}</label>)}</div>
              <div className="utility-member-options">
                ...
                <button type="button" className="paper-button" disabled={!persisted} title={!persisted ? (zh ? "请先保存 System Intelligence 设置。" : "Save System Intelligence first.") : undefined} onClick={() => setCredentialMember(member)}>{credential?.configured ? (zh ? "替换 Key" : "Replace key") : (zh ? "配置 Key" : "Configure key")}</button>
                {credential?.configured && <button type="button" className="paper-button" onClick={() => void onClearCredential(member.id)}>{zh ? "清除 Key" : "Clear key"}</button>}
              </div>
              <footer><span>{zh ? "Quota / Health 会在 Phase 2 接入" : "Quota / Health arrives in Phase 2"}</span><button type="button" className="key-group-delete-link" onClick={() => patch({ members: config.members.filter((_, itemIndex) => itemIndex !== index) })}>{zh ? "移除 member" : "Remove member"}</button></footer>
            </article>
          );
        })}
      </div>
      ...
      <section className="utility-paid-fallback">
        <header><div><span>PAID FALLBACK</span><strong>OpenRouter only</strong></div><label className="runtime-toggle"><input type="checkbox" checked={config.paid_fallback.enabled} onChange={(event) => patch({ paid_fallback: { ...config.paid_fallback, enabled: event.currentTarget.checked } })} />{zh ? "允许付费兜底" : "Allow paid fallback"}</label></header>
        <div className="runtime-provider-grid">
      ...
      {credentialMember && <CredentialModal utility={{ memberId: credentialMember.id, name: credentialMember.name, provider: credentialMember.provider, model: credentialMember.model }} onClose={() => setCredentialMember(null)} onConfigured={() => void onRefreshCredentials()} />}
    </section>
  );
}
